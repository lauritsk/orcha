...
def parse_commit_message(raw: str) -> CommitMessage:
    ...
    try:
        ...
        echo_err(f"orcha: pi message output is not valid JSON: {error}")
        abort(1)

    if not isinstance(data, dict):
        echo_err("orcha: pi message output must be a JSON object")
        abort(1)
    ...
    if not title:
        echo_err("orcha: pi message title is empty")
        abort(1)
    if "\n" in title or "\r" in title:
        echo_err("orcha: pi message title must be one line")
        abort(1)
    if len(title) > MESSAGE_TITLE_LIMIT:
        echo_err(f"orcha: pi message title exceeds {MESSAGE_TITLE_LIMIT} characters")
        abort(1)
    if not body:
        echo_err("orcha: pi message body is empty")
        abort(1)
    if len(body) > MESSAGE_BODY_LIMIT:
        echo_err(f"orcha: pi message body exceeds {MESSAGE_BODY_LIMIT} characters")
        abort(1)
    if "\0" in title or "\0" in body:
        echo_err("orcha: pi message contains a NUL byte")
        abort(1)
...
def _string_field(data: dict[str, Any], field: str) -> str:
    ...
    if isinstance(value, str):
        return value
    echo_err(f"orcha: pi message field {field!r} must be a string")
    abort(1)
